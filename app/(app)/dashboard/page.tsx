...
export default async function DashboardPage() {
  ...
  const dueSoonByIso = new Date(today.getTime() + DUE_SOON_DAYS * 24 * 60 * 60 * 1000)
    .toISOString()
    .slice(0, 10)
  const startOfMonthIso = new Date(today.getFullYear(), today.getMonth(), 1)
    .toISOString()
    .slice(0, 10)

  const [{ data: balances }, { data: recent }, { count: draftCount }, { data: monthPayments }] =
    await Promise.all([
      supabase
      ...
      supabase
        ...
        .eq('kind', 'invoice')
        .eq('status', 'draft'),
      supabase.from('payments').select('invoice_id, amount_paise, tds_paise').gte('paid_on', startOfMonthIso),
    ])
  ...
  for (const b of balances ?? []) {
    ...
    } else if (b.due_date && b.due_date >= todayIso && b.due_date <= dueSoonByIso) {
